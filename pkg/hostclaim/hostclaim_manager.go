...
func (m *HostManager) setBmhSpec(ctx context.Context, bmh *metal3api.BareMetalHost) (bool, error) {
	...
	if !reflect.DeepEqual(bmh.Spec.UserData, ref) {
	...
	if !reflect.DeepEqual(bmh.Spec.MetaData, ref) {
	...
	if !reflect.DeepEqual(bmh.Spec.NetworkData, ref) {
	...
	if annotValue, ok := hostMap[metal3api.DetachedAnnotation]; ok {
		...
		if compatiblePolicies, err := m.GetCompatiblePolicies(ctx, bmhNs); err != nil {
			...
			func(hdp *metal3api.HostDeployPolicy) bool {
				return hdp.Spec.AllowsDetaching
			},
		) {
		...
		args := metal3api.DetachedAnnotationArguments{DeleteAction: inputArgs.DeleteAction, SetByHostClaim: true}
		...
		args := metal3api.DetachedAnnotationArguments{}
		if err := json.Unmarshal([]byte(annotValue), &args); err != nil || !args.SetByHostClaim {
