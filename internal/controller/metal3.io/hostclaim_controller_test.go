	...
	func() {
		DescribeTable("Test Reconcile",
			func(tc testCaseHostClaimReconcile) {
				ctx := context.TODO()
				hdp := NewHostdeploypolicy("hdp", baremetalNamespace).AcceptNames([]string{HostclaimNamespace}).Build()
				ns1 := NewNamespace(HostclaimNamespace).Build()
				ns2 := NewNamespace(baremetalNamespace).Build()
			...
			Entry("Starting Reconciliation with a non ready BMH (no association)", testCaseHostClaimReconcile{
				...
				BareMetalHost:     NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateRegistering).Build(),
				ExpectRequeue:     true,
			...
			Entry("Starting Reconciliation with a ready BMH", testCaseHostClaimReconcile{
				...
				BareMetalHost:     NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateAvailable).SetConsumerRef(defaultConsumerRef).Build(),
			...
			Entry("Starting Reconciliation with a ready BMH but not all secrets", testCaseHostClaimReconcile{
				HostClaim:             NewHostclaim(HostclaimName).SetAssociatedBMH("ns", "bmh").SetUserData("sec-not-provided").Build(),
				BareMetalHost:         NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateAvailable).SetConsumerRef(defaultConsumerRef).Build(),
				AssociationReason:     metal3api.BareMetalHostAssociatedReason,
			...
			Entry("Updating an associated BMH", testCaseHostClaimReconcile{
				HostClaim:             NewHostclaim(HostclaimName).SetAssociatedBMH("ns", "bmh").Build(),
				BareMetalHost:         NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateProvisioned).SetConsumerRef(defaultConsumerRef).Build(),
				AssociationReason:     metal3api.BareMetalHostAssociatedReason,
			...
			Entry("Updating a ready BMH", testCaseHostClaimReconcile{
				HostClaim:             NewHostclaim(HostclaimName).SetAssociatedBMH("ns", "bmh").SetCondition(metal3api.ProvisionedCondition, true, "any").Build(),
				BareMetalHost:         NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateProvisioned).SetCondition(metal3api.ProvisionedCondition, true, "any").SetConsumerRef(defaultConsumerRef).Build(),
				AssociationReason:     metal3api.BareMetalHostAssociatedReason,
				SynchronizationReason: metal3api.ConfigurationSyncedReason,
				HasFinalizer:          true,
				ExpectReady:           true,
			...
			Entry("Pausing an associated BMH", testCaseHostClaimReconcile{
				HostClaim:         NewHostclaim(HostclaimName).SetAssociatedBMH(baremetalNamespace, baremetalName).SetAnnotations(map[string]string{metal3api.PausedAnnotation: ""}).Build(),
				BareMetalHost:     NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateAvailable).SetConsumerRef(defaultConsumerRef).Build(),
			...
			Entry("Fail to pause an associated BMH (simulated error)", testCaseHostClaimReconcile{
				HostClaim:         NewHostclaim("other").SetAssociatedBMH(baremetalNamespace, baremetalName).SetAnnotations(map[string]string{metal3api.PausedAnnotation: ""}).Build(),
				BareMetalHost:     NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateAvailable).SetConsumerRef(defaultConsumerRef).Build(),
				AssociationReason: metal3api.PauseAnnotationSetFailedReason,
			}),
			Entry("Deletion standard case", testCaseHostClaimReconcile{
				HostClaim: NewHostclaim(HostclaimName).SetAssociatedBMH(baremetalNamespace, baremetalName).
					SetFinalizer([]string{metal3api.HostClaimFinalizer}).Build(),
				BareMetalHost:        NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateAvailable).SetConsumerRef(defaultConsumerRef).SetCleaningMode("metadata").Build(),
			...
			Entry("Starting Reconciliation with a ready BMH (conflict on bmh update)", testCaseHostClaimReconcile{
				HostClaim:         NewHostclaim(HostclaimName).Build(),
				BareMetalHost:     NewBaremetalhost(baremetalName, baremetalNamespace, metal3api.StateAvailable).Build(),
			...
			Entry("Deletion with conflict", testCaseHostClaimReconcile{
				...
				BareMetalHost: NewBaremetalhost(
					baremetalName, baremetalNamespace, metal3api.StateAvailable).SetConsumerRef(defaultConsumerRef).Build(),
				DeleteRequested:      true,
				WithErrorAfter:       1,
				ExpectRequeue:        true,
				ExpectDeleteSucceeds: false,
